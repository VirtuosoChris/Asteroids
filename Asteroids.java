...
public class Asteroids extends JApplet implements Runnable,KeyListener{
	...
	public void spawnChildren(Asteroid ast, FlyingShit x){
		...
		if(ast.material.equals(Material.ICE))return;
	...
	while(true){
	 ...
	 if(!paused){
	  ...
	  try{
	  	...
	  	if(!menu.ready){thrust.stop();continue;}
	    ...
	    //handle shield consumption
	     ...
	     //update and move the player's ship
	      if(playerShip!= null && playerShip.isAlive()){
	      	...
	      	if(up){playerShip.accelerate();}
	  		
	  	  	if(down){playerShip.decelerate();}
	  	
	        	if(left){playerShip.rotate(-Math.PI/25);}
	  	
	  		if(right){playerShip.rotate(Math.PI/25);}
	      ...
	      for(Bullet b : UFO.UfoBullets){
	      	...
	      	if(b.hit(playerShip)){
	      		...
	      		if(!godmode && !playerShip.shielded){
	      		  ...
	      		  else{playerShip.removeUpgrade(Ship.ARMOR);}
	...
	public void keyPressed(KeyEvent k){
	  ...
	  if(k.getKeyChar() == 'N' || k.getKeyChar() == 'n'){
	  	...
	  	if(!sound)thrust.stop();
	  	if(sound && up)thrust.loop();
	  }
	  ...
	  if(paused||!playerShip.isAlive())return; //if we're paused or dead don't let the user cheat
	...
	public void paint(Graphics g){
		...
		if(g == null)return;
		if(offScrnBfr == null)return;
		if(!menu.ready){super.paint(g);return;}
